...
RC next (RM_ScanHandle *scan, Record *record){
    ...
    while(rmScanMgmt.count<tblmgmt_info.totalRecordInTable){
        ...
        evalExpr(record, (scan->rel)->schema, rmScanMgmt.cond, &queryExpResult);
        if(queryExpResult->v.boolV ==1){rmScanMgmt.recID.slot= rmScanMgmt.recID.slot + 1; return RC_OK;}
...
void strRepInt(int j,int val,  char *intStr){
    int q = val;
    int last = j;
    while (q > 0 && j >= 0) { intStr[j] += q % 10; q /= 10; j--;}
    intStr[last+1] = '\0';
}
...
void schemaReadFromFile(RM_TableData *rel, BM_PageHandle *h){
    ...
    DataType *dt =   (DataType*) getAtributesDtType(atrMetadata,totalAtribute);
    int *sizes = getAtributeSize(atrMetadata,totalAtribute);
    int *keys =   extractKeyDt(readAttributeKeyData(metadata),totalKeyAtr);
    int *pageSolt = extractFirstFreePageSlot(readFreePageSlotData(metadata));
    char **cpNames = (char **) malloc(sizeof(char*) * totalAtribute);
    DataType *cpDt = (DataType *) malloc(sizeof(DataType) * totalAtribute);
    int *cpSizes = (int *) malloc(sizeof(int) * totalAtribute);
    int *cpKeys = (int *) malloc(sizeof(int)*totalKeyAtr);
    ...
    free(keys);
    free(schema_name);
    rel->schema=createSchema(totalAtribute, cpNames, cpDt, cpSizes, totalKeyAtr, cpKeys);;
    rel->name =cpSchemaName;
    tblmgmt_info.rm_tbl_data = rel;
    tblmgmt_info.sizeOfRec =  getRecordSize(rel->schema) + 1;   //
    tblmgmt_info.blkFctr = (PAGE_SIZE / tblmgmt_info.sizeOfRec);
    tblmgmt_info.firstFreeLoc.page =pageSolt[0];
    tblmgmt_info.firstFreeLoc.slot =pageSolt[1];
    tblmgmt_info.totalRecordInTable = extractTotalRecordsTab(&metadata);
...
int readTotalAttributes(char *scmData){
    ...
    memset(strNumAtr,'\0',sizeof(int)*2);
    int i=0; int j=0;
    for(i=0; scmData[i] != '|'; i++){} i++;
    while(scmData[i] != '[') strNumAtr[j++]=scmData[i++];
    strNumAtr[j]='\0';
    return atoi(strNumAtr);
}
...
char * readAttributeMeataData(char *scmData){
    ...
    int i=0; int j=0;
    while(scmData[i] != '[') i++; i++;
    for(j=0;scmData[i] != ']'; j++) atrData[j] = scmData[i++];
    atrData[j]='\0';
    return atrData;
}
...
char * getSingleAtrData(char *scmData, int atrNum){
    ...
    while(count<=atrNum) if(scmData[i++] == '(') count++;
    for(j=0;scmData[i] != ')'; j++) atrData[j] = scmData[i++];
    atrData[j]='\0';
    return atrData;
}
...
char * extractName(char *data){
    ...
    memset(name,'\0',sizeof(char)*10);
    int i;
    for(i=0; data[i]!=':'; i++) name[i] = data[i];
    name[i]='\0';
    return  name;
}
...
int extractDataType(char *data){
    ...
    int i; int j;
    for(i =0 ; data[i]!=':'; i++){} i++;
    for(j=0; data[i]!='~'; j++) dtTp[j]=data[i++];
    dtTp[j]='\0';
    int dt =atoi(dtTp);
    free(dtTp);
    return dt;
...
int extractTypeLength(char *data){
    ...
    int i; int j;
    for(i =0 ; data[i]!='~'; i++){} i++;
    for(j=0; data[i]!='\0'; j++) dtLen[j]=data[i++];
    dtLen[j]='\0';
    int dt =atoi(dtLen);
    free(dtLen);
    return dt;
...
int * extractKeyDt(char *data,int keyNum){
    ...
    int i=0; int j=0;
    for(int k=0; data[k]!='\0'; k++){
        if(data[k]==':' ){ values[j]=atoi(val); memset(val,'\0',sizeof(int)*2); i=0; j++; }
        else val[i++] = data[k];
    }
    values[keyNum-1] =atoi(val);
    return  values;
...
void printPageData(char *pageData){
    printf("\n Prining page Data ==>");
    for(int i=0; i<PAGE_SIZE; i++) printf("%c",pageData[i]);
    printf("\n exiting ");
    printf("\n exiting ");
}